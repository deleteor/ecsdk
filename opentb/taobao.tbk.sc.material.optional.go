...
import "git.hp6h2.cn/gobin/commons/gtype/boolx"
...
type TbkScMaterialOptionalRequest struct {
	...
	IsOverseas boolx.Bool `json:"is_overseas" form:"is_overseas"`
	// IsTmall	false	false	商品筛选-是否天猫商品。true表示属于天猫商品，false或不设置表示不限
	IsTmall boolx.Bool `json:"is_tmall" form:"is_tmall"`
	...
	HasCoupon boolx.Bool `json:"has_coupon" form:"has_coupon"`
	...
	IncludeRfdRate boolx.Bool `json:"include_rfd_rate" form:"include_rfd_rate"`
	// IncludeGoodRate	false	true	商品筛选-好评率是否高于行业均值。True表示大于等于，false或不设置表示不限
	IncludeGoodRate boolx.Bool `json:"include_good_rate" form:"include_good_rate"`
	// IncludePayRate30	false	true	商品筛选(特定媒体支持)-成交转化是否高于行业均值。True表示大于等于，false或不设置表示不限
	IncludePayRate30 boolx.Bool `json:"include_pay_rate_30" form:"include_pay_rate_30"`
	// NeedPrepay	false	true	商品筛选-是否加入消费者保障。true表示加入，false或不设置表示不限
	NeedPrepay boolx.Bool `json:"need_prepay" form:"need_prepay"`
	// NeedFreeShipment	false	true	商品筛选-是否包邮。true表示包邮，false或不设置表示不限
	NeedFreeShipment boolx.Bool `json:"need_free_shipment" form:"need_free_shipment"`
